...
- (void) _bootstapApplicationEventPort
{
	/*
	...
	if (kr != KERN_SUCCESS)
	{
		MSLog(@"'%@' does not seem to have an active MiG server. Killing.",_bundleIdentifier);
		[self kill];
	}
	else 
	{
...
- (void) _setActivationSetting:(int)setting
{
	...
	MXSLog(@"Record size: %d",sizeof(GSEventActivation));
	 ...
	 * XXX
	 *
	 * Orientation is a bit weird.
	 * Some applications take it correctly.
	 * Some do the exact opposite.
	 * UIKit does take it correctly but the apps themselves act weirdly.
	 */
...
- (void) _watchDogTimerFired
{
	...
	if (kill([_process pid], 0) == 0)
	{
		...
		if (!_isRunning)
		{
			[self _bootstapApplicationEventPort];
			[self activate];
		}
...
- (void) _startWatchDogTimer
{
	...
	if (_watchDogTimer == NULL)
	{
		...
		_watchDogTimer = [NSTimer scheduledTimerWithTimeInterval:1.5
														  target:self

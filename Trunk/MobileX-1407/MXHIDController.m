...
void MXHIDHandleKBEvent(IOHIDEventRef event)
{
	...
	printf("[hid] keyboard {usage: %p, isDown: %d}\n",
		   (void*)IOHIDEventGetIntegerValue(event, kIOHIDEventFieldKeyboardUsage),
		   isDown);
	...
	switch (IOHIDEventGetIntegerValue(event, kIOHIDEventFieldKeyboardUsage))
	{
		...
		case kDevBtnHome: /* Home button */
			...
			break;
		case kDevBtnLock: /* Lock button */
			directButtonEvent(kAtvBtnMenu, isDown);
			//record.type = isDown ? kGSEventLockButtonDown : kGSEventLockButtonUp;
			break;
		case kDevBtnVolDown:
			//record.type = isDown ? kGSEventVolumeDownButtonDown : kGSEventVolumeDownButtonUp;
			directButtonEvent(kAtvBtnDown, isDown);
			break;
		case kDevBtnVolUp: /* vol up  */
			//record.type = isDown ? kGSEventVolumeUpButtonDown : kGSEventVolumeUpButtonUp;
			directButtonEvent(kAtvBtnUp, isDown);
			break;			
		default:
			directButtonEvent(IOHIDEventGetIntegerValue(event, kIOHIDEventFieldKeyboardUsage), isDown);
			[pool release];
			return;
			break;
	}
...
void hid_xxHandleMultitouchEvent(IOHIDEventRef event)
{
	...
	if (children && (childrenCount = CFArrayGetCount(children))) 
	{
		...
		{
			if ([[MXMultitouchHandler shared] handleMultitouchEventInPort:destinationPort
...
void MXHIDHandleEvent (void* target, void* refcon, IOHIDServiceRef service, IOHIDEventRef event)
{
	...
	uint32_t event_type = IOHIDEventGetType(event);
	
	//printf("[hid] handling iohidevent of type %d\n", event_type);
	
	if (event_type == kIOHIDEventTypeDigitizer)
	{
		hid_handleMultitouchEvent(event);
	}
	else if (event_type == kIOHIDEventTypeKeyboard)
	{
		MXHIDHandleKBEvent(event);
	}
	else if (event_type == kIOHIDEventTypeAccelerometer)
	{
		MXHIDHandleACEvent(event);
	}
	else if (event_type == kIOHIDEventTypeAmbientLightSensor)
	{
		
	}
	else if (event_type == kIOHIDEventTypeButton)
	{
		printf("[hid] button {mask: %d, number: %d}\n", IOHIDEventGetIntegerValue(event, kIOHIDEventFieldButtonMask),
			   IOHIDEventGetIntegerValue(event, kIOHIDEventFieldButtonNumber));
	}
	else {
...
IOHIDEventSystemRef _MXHIDStartInternal(BOOL isEventPump)
{
	...
	else if ([[[MXDevice currentDevice] model] hasPrefix:@"AppleTVfaux"]) {
		_deviceType = 1;
	}
	...
	else 
	{
		...
		if (result)
		{
			...
			if (_deviceType == 3) {
				MXLog(@"Configuring HID for AppleRemote ...",_deviceType);
				
				IOHIDServiceRef rem = 
				MXHIDGetService(1, 0x80);
				
				MSIndent();
				
				MSLog(@"%@", rem);
				MSLog(@"SIL state (com.apple.AppleTVHID.systemEvent): %@", IOHIDServiceGetProperty(rem, CFSTR("com.apple.AppleTVHID.systemEvent")));
				
				MSLog(@"Setting SIL to 'on' ...");
				
				IOHIDServiceSetProperty(rem, CFSTR("com.apple.AppleTVHID.systemEvent"), CFSTR("on")); 
				
				IOHIDServiceSetProperty(rem, CFSTR("com.apple.AppleTVHID.logging"), CFSTR("console")); 
				
				MXHIDSetDebugStateForService(rem, TRUE);
